...
export async function generateAngularApplicationGenerator(
  ...
  tree.delete(
    `apps/${options.domain}/${options.name}/src/app/nx-welcome.component.ts`
  )
  ...
  tree.write(
    ...
    tree
      ...
      .toString()
      .replace(
        '<business-tools-monorepo-nx-welcome></business-tools-monorepo-nx-welcome>',
        ''
      )
  )
  tree.write(
    appCompTs,
    tree.read(appCompTs).toString().replace('NxWelcomeComponent, ', '')
  )
  tree.write(
    appCompTs,
    tree
  ...
  tree.write(
    appCompSpec,
    tree.read(appCompSpec).toString().replace('NxWelcomeComponent, ', '')
  )
  tree.write(
    appCompSpec,
    tree
